...
# pre-pop state and returns the state after the balloon is popped. The pre-pop state will contain at most a single balloon, whose 
# size is represented by the only non-zero element.
...
def pop(state):
	return[max(state) - abs(max(state) - i) for i in range(len(state))]
